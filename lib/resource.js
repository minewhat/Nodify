(function() {
  ...
  Resource = (function(_super) {
    ...
    Resource.prototype.__request__ = function(url, slug, method, fields, callback) {
      ...
      try{
        storename = url.split("https://")[1].split(".myshopify")[0]
      }catch (e)
      {

      }
      if(storename && this.store && this.store[storename])
       token = this.store[storename];
      else
      ...
      options = {
        ...
        method: method,
        json: slug !== 'oauth',
        headers: this.oauth === true && (token != null) ? {
      ...
      return request(options, function(err, response, body) {
        ...
        if(response)
          status = parseInt(response.statusCode);else{
          status = 403;
        }
        if (status >= 300) {
    ...
    Resource.prototype.setOAuthToken = function(store,oauth_token) {
      if(!this.store)this.store = {}
      this.store[store] = oauth_token;
      this.oauth_token = oauth_token;
      return this.oauth = oauth_token != null;
    };
